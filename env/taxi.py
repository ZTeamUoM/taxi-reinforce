...
class TaxiEnv(gym.Env):
    ...
    def __init__(self, state_dict=None):
        ...
        high = np.array([np.finfo(np.float32).max, np.finfo(np.float32).max ,24, np.finfo(np.float32).max], dtype=np.float32)
    ...
    def reset(self):
        ...
        self.state = np.array([0,0,20,0], dtype=np.float32)
        self._episode_ended = False
